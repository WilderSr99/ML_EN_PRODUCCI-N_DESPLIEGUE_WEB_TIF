...
if "mto_pim" not in st.session_state:
    st.session_state.mto_pim = 0.0

if "mto_dev_01_06" not in st.session_state:
    st.session_state.mto_dev_01_06 = 0.0

if "mto_dev_07" not in st.session_state:
    st.session_state.mto_dev_07 = 0.0

if "mto_dev_08" not in st.session_state:
...
st.markdown("### Predicción de ejecución acumulada al mes de setiembre, empleando variables vinculadas a la ejecución presupuestal.")
...
mto_pim = st.sidebar.number_input(
    "Monto PIM (Min=0)",
    min_value=0.0,
    key="mto_pim"
)
...
mto_dev_01_06 = st.sidebar.number_input(
    "Monto devengado al 1er semestre",
    min_value=0.0,
    key="mto_dev_01_06"
)

# Entrada del usuario para monto devengado en el mes de julio
mto_dev_07 = st.sidebar.number_input(
    "Monto devengado en Julio",
    min_value=0.0,
    key="mto_dev_07"
)

# Entrada del usuario para monto devengado en el mes de agosto
mto_dev_08 = st.sidebar.number_input(
    "Monto devengado en Agosto",
    min_value=0.0,
    key="mto_dev_08"
)
